...
const SearchSection = () => {
  const { medicalTests, bookTest, currentUser, setActiveSection, currentTest } = useApp();
  ...
  }, [medicalTests, searchTerm, locationFilter, priceFilter, categoryFilter]);
          ...
          filteredTests.map(test => (
            <div key={test.id} className="test-card">
              <div className="test-info">
                <h3>{test.name}</h3>
                <p className="hospital">{test.hospital}</p>
                <p className="location">
                  <i className="fas fa-map-marker-alt"></i> {test.location} • {test.category}
                </p>
